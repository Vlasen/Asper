...
def main():
    ...
    x64_llvm_path = os.path.abspath(os.path.join(ndk_root))
    ...
    config.set('DEFAULT', anysdk_common_dir)
    ...
    if '3.' in llvm_path:
        if '3.6' in llvm_path:
            config.set('DEFAULT', 'clang_include', 'lib/clang/5.0.1/include')
        else:
            config.set('DEFAULT', 'clang_include', 'lib/clang/5.0.1/include')
    else:
        config.set('DEFAULT', 'clang_include', 'lib64/clang/5.0.1/include')
    ...
    try:
        ...
        output_dir = '%s/cocos/scripting/js-bindings/bindings/auto' % project_root
        ...
        for key in cmd_args.keys():
            ...
            command = '%s %s %s -s %s -t %s -o %s -n %s' % ("python", generator_py, cfg, args[0], target, new_output_dir, args[1])
            _run_cmd(command)
